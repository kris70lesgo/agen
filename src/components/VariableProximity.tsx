  ...
  (props, ref) => {
    ...
    useAnimationFrame(() => {
      ...
      letterRefs.current.forEach((letterRef, index) => {
        ...
        const poweredFalloff = Math.pow(
          Math.max(falloffValue, 0),
          intensity,
        );
        const newSettings = parsedSettings
          .map(({ axis, fromValue, toValue }) => {
            const interpolatedValue =
              fromValue + (toValue - fromValue) * poweredFalloff;
